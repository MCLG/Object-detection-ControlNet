...
class GaussianMixture(torch.nn.Module):
    ...
    def fit(self, x_, delta=1e-3, n_iter=100, warm_start=False):
        """
        Fits model to the data.
        args:
            x_:          torch.Tensor (n, d) or (n, k, d)
        options:
            delta:      float
            n_iter:     int
            warm_start: bool
        ...
        #print(f'{x_.requires_grad=}')
        #if x_.requires_grad is True :
        #    x_.retain_grad()
        
        x = self.check_size(x_)
