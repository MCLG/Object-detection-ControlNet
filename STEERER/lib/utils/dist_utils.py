...
def is_distributed() -> bool:
    ...
    print(f'[STEERER.lib.utils in is_distributed()] torch_dist.is_available()={torch_dist.is_available()} - torch_dist.is_initialized() = {torch_dist.is_initialized()}')
...
def _init_dist_pytorch(backend, **kwargs) -> None:
    ...
    print(f'[current location : "/home/luk02485/development/ControlNet/STEERER/lib/utils/dist_utils.py:73-74"] device is being set via rank % num_gpus = {rank} % {num_gpus} = {rank % num_gpus}')
...
def _init_dist_mpi(backend, **kwargs) -> None:
    ...
    print(f'[current location : "/home/luk02485/development/ControlNet/STEERER/lib/utils/dist_utils.py:95:"] device is set as local_rank={local_rank}')
...
def _init_dist_slurm(backend, port=None) -> None:
    ...
    print(f'[current location : "/home/luk02485/development/ControlNet/STEERER/lib/utils/dist_utils.py:121:"] device is set as cuda.set_device(proc_id % num_gpus)={proc_id}%{num_gpus}={proc_id % num_gpus}')
...
def get_comm_device(group: Optional[ProcessGroup] = None) -> torch.device:
    ...
    print(f'[current location : "/home/luk02485/development/ControlNet/STEERER/lib/utils/dist_utils.py:398:" in "get_comm_device()"] \n'
        f' backend = {backend}')
    if backend == 'hccl':
