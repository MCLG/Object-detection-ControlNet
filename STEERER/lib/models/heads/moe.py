...
class one_branch_output(nn.Module):
    ...
    '''def channels_down_wrapper(self, input : tuple) ->  List[torch.Tensor] :
 
        if len(input) > 1 :
            print(f'Unknown occurence in {__file__.__qualname__}')
            out_list = [self.channels_down(out_tensor) for out_tensor in input ]
            return out_list 
        
        out_list = list()
        for tensor in input :
            with torch.set_grad_enabled(True) :
                print(f'fetched tensor, {tensor.requires_grad=}')
                tensor = self.channels_down[0](tensor).unsqueeze(0)
                print(f'after first transformation : {tensor.requires_grad=}')
                tensor = self.channels_down[1](tensor)
                print(f'after second transformation : {tensor.requires_grad=}')
                tensor = self.channels_down[2](tensor)
                print(f'after third transformation : {tensor.requires_grad=}')
                out_list.append(tensor)
        return out_list'''

    def forward(self, current_stage, last_stage=None, counter=None,out_branch=False,no_distangle=False):
        
        #TODO: use activate_grad variable on last_stage.requires_grad too
        try :
            activate_grad = current_stage.requires_grad
        except AttributeError :
            activate_grad = current_stage[0].requires_grad
        ...
        if no_distangle:
            ...
            #with torch.set_grad_enabled(activate_grad and torch.is_grad_enabled()) :
            current_stage = self.channels_down(current_stage)
            ...
            if last_stage is None:
                ...
                if out_branch:
                    ...
                    #with torch.set_grad_enabled(last_stage.requires_grad and torch.is_grad_enabled()) :
                    last_stage = F.interpolate(last_stage, scale_factor=2,mode='nearest')
                    last_stage = self.modulation_layer_big(last_stage)
                    ...
                    out_put = counter(torch.cat([current_stage, last_stage_large],1)) #### the counter here is a copy of self.counter and is likely responsible to setting grad to false
                    ...
                    if self.fusion_method == "cat":
                        ...
                        current_stage = current_stage+last_stage
                                #out_put, current_stage = torch.utils.checkpoint.checkpoint(self.not_out_branch, (last_stage,current_stage,counter), use_reentrant=False) 

            return out_put, current_stage

        '''    def not_out_branch(self, inputs) :
        last_stage, current_stage, counter = inputs
...
class upsample_module(nn.Module):
    ...
    '''def multi_w_grad_checkpointing(self, inputs) :
        index, current_stage, last_stage, counter_,out_branch = inputs
        return self.multi_outputs[index](current_stage[-(index+1)],last_stage,
                                                        counter_,out_branch=True)
    def multi_outputs_wrapper(self, index, current_stage, last_stage, counter_,out_branch=False) :  #CHANGE FOR 2 INDEXESSS
        if out_branch :
            return self.multi_outputs[index](current_stage[-(index+1)],last_stage,
                                                        counter_,out_branch=True)
        inputs = (index, current_stage, last_stage, counter_,out_branch)
        return torch.utils.checkpoint.checkpoint(self.multi_w_grad_checkpointing, inputs = False)'''
    ...
    def forward(self,in_list, counter, counter_copy):
        ...
        output, last_stage = self.multi_outputs[0](current_stage=in_list[-1], last_stage=None,counter=counter_copy)
        #output, last_stage =  self.multi_outputs_wrapper(index=0,current_stage=in_list, last_stage=None,counter_=counter_copy)#self.multi_outputs[0](in_list[-1],
                                                    #last_stage=None,
        ...
        for i in range(1,self.num_resolutions):
            #two next calls are instances of one_branch_output.forward
